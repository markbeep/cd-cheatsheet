...
To determine least fixed point (places where $\phi$-functions have to be inserted)
for $N = \{X,Y\}$:
\begin{compactitem}
	\item Find $DF[N] = DF_0[\{X,Y\}] = \{X,Y,A\}$
	\item Find $DF_1[\{X, Y, A, B\}] = \{X,Y,A,B,C\}$
	\item Continue until same: $DF_2[\textcolor{blue}{\{X,Y,A,B,C\}}] = \textcolor{blue}{\{X,Y,A,B,C\}}$
