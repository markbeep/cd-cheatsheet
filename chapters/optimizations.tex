...
For a back edge $n \rightarrow h$, $h$ is the header. Loops with the same header can be merged (i.e $\{A,B,C\}, \{A,D\}$ can be merged to $\{A,B,C,D\}$). A \textbf{loop} contains at least 1 back edge (\textbf{back edge} = target dominates the source). We can formally define a loop as $L(n \to h) = \{ n' | n \text{ is reachable from } n' \text{ in } G \backslash\{h\}  \} \cup \{h\}$.\medskip
